...
@Component({
  selector: 'app-kiosk-designer-menu-items',
  templateUrl: './menu-items.component.html',
  styleUrls: ['./menu-items.component.scss', '../shared.scss'],
})
export class MenuItemsComponent implements OnInit {
  ...
  private async fetchData() {
    ...
    modifierGroups.forEach((modifierGroup) => {
      modifierIds = [
        ...modifierIds,
        ...modifierGroup.modifiers.map((modifier) => modifier.menuItem.id),
      ];
    });
    this.items = items.filter((item) => !modifierIds.includes(item.id));
    this.refreshDisplayedItems();
  }
